...
public class AutoAlign implements Command {
    ...
    double xPwr;
    double distL, distR;
    double sideAlignMaxDist, sideAlignMinDist;
    boolean alignLeft;
    double timeMarker;
    double startTime;
    ...
    public AutoAlign(CrabRobot robot, Drivetrain drive, double align_power, boolean alignLeft, Telemetry telemetry){
        this.robot = robot;
        ...
        xPwr= align_power;
        sideAlignMinDist = CONE_OFFSET + 4;
    }
    @Override
    public void start() {
    ...
    public void update() {
        ...
        if (state == 0) { // Move forward till in pole range
            ...
            if (       distL < DIST_THRESHOLD
                ...
                if (this.alignLeft) {
                    if (distL < NO_CONE_THRESHOLD) { // There is cone on left
                        sideAlignMaxDist = POLE_OFFSET - CONE_OFFSET;
                    } else {
                        sideAlignMaxDist = POLE_OFFSET;
                    }
                } else {// alignRight
            ...
            if (this.alignLeft) {
                ...
                } else if (curr_dist < sideAlignMinDist) {
                    mecanumDrive.setDrivePower(new Pose2d(0,-xPwr,0));
                } else {
                    mecanumDrive.setDrivePower(new Pose2d(0, 0,0));
                    timeMarker = clock.seconds();
                ...
                double curr_dist = robot.robotdistancesensor.dsL;
                if (curr_dist > sideAlignMaxDist) {
                    mecanumDrive.setDrivePower(new Pose2d(0,-xPwr,0));
                } else if (curr_dist < sideAlignMinDist) {
                    mecanumDrive.setDrivePower(new Pose2d(0,xPwr,0));
    ...
    public void stop() {
        mecanumDrive.setDrivePower(new Pose2d(0,0,0));
        robot.removeSubsystem((Subsystem) robot.robotdistancesensor);
    }

    @Override
    public boolean isCompleted() {
        return (   (state == 2)
        || (clock.seconds()- startTime > 2.0)); // timeout in 2s
    }
}
